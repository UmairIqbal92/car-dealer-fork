import Link from "next/link"

export default function NewsletterBanner() {
  return (
    <section className="relative py-0 px-4 md:px-6 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="relative -mb-24 md:-mb-28 z-10">
          <div
            className="rounded-xl shadow-2xl py-6 px-6 md:py-8 md:px-12 text-white max-w-6xl mx-auto"
            style={{ backgroundColor: "#C74B3F" }}
          >
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-10">
              {/* Logo */}
              <div className="flex-shrink-0">
                <img src="/logo.png" alt="Car Junction LLC Logo" className="h-14 md:h-16 w-auto bg-white p-2 rounded" />
              </div>

              {/* Message - takes more space */}
              <div className="flex-grow text-center md:text-left">
                <h3 className="text-xl md:text-2xl font-bold mb-2">WE CAN GET YOU FINANCED</h3>
                <p className="text-sm md:text-base text-red-50">
                  Good Credit, Bad Credit, First Time Buyer? Our lenders work with all types of scores and situations.
                </p>
              </div>

              {/* CTA Button */}
              <div className="flex-shrink-0">
                <Link href="/apply">
                  <button className="bg-black text-white px-8 py-4 rounded-lg font-bold hover:bg-gray-800 transition-colors text-base whitespace-nowrap">
                    Apply Online
                  </button>
                </Link>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}
